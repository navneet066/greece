			...
			<table class="table table-striped table-bordered table-hover" id="package_index">
				...
				<?php foreach ($packages as $key => $value) {
					...
					<tr>
						...
						<td><?php echo $value['Package']['price']?></td>
						<td><?php echo $value['Package']['valid_days']?></td>
						<td>
							<?php
							$updateProfile = '<i class="fa fa-pencil-square-o bigger-130"></i>';
							echo $this->Html->link(
								$updateProfile,
								array("controller"=>"packages","action" => "update", $id),
								array("class" => "text-success", "escape"=> false,'title'=>__('Update Package')));
							?>
							|
							<?php
							...
							echo $this->Form->postLink(
								$removeText,
								array("controller"=>"packages","action" => "remove", $id),
